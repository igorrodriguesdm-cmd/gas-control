...
def login():
    ...
    if not nome_operador:
        return jsonify({"erro": "O preenchimento do seu nome é obrigatório para auditoria."}), 400
...
def cadastrar_tudo():
    ...
    if tipo_cadastro == 'deposito':
        ...
        if not nome or not usuario_login or not senha_login:
            return jsonify({"erro": "Preencha os campos obrigatórios do depósito."}), 400
        ...
        DATABASE["caminhoes"].append({"id": novo_id, "placa": placa, "capacidade": capacidade})
...
def gerar_relatorios():
    ...
    vendas_filtradas = [
        v for v in DATABASE["vendas"] 
        if (not depositos_selecionados or v["deposito_id"] in depositos_selecionados)
    ]
...
def dados_deposito(dep_id):
    ...
    vendas_hoje = [v for v in DATABASE["vendas"] if v["deposito_id"] == dep_id and v["data"].startswith(hoje_str)]
    trocas_hoje = [t for t in DATABASE["trocas"] if t["deposito_id"] == dep_id and t["data"].startswith(hoje_str)]
...
@app.route('/api/deposito/<int:dep_id>/historico', methods=['GET'])
def historico_deposito(dep_id):
    vendas_dep = [v for v in DATABASE["vendas"] if v["deposito_id"] == dep_id]
    historico_formatado = [{
        "data": v["data"],
        "operacao": f"Venda ({v['quantidade']} un - {v['pagamento']})",
...
def transferir_estoque():
    ...
    if origem_id == destino_id:
        return jsonify({"erro": "Depósito de origem e destino não podem ser iguais."}), 400
    ...
    if origem[tipo_botijao] < quantidade:
        return jsonify({"erro": f"Estoque insuficiente de botijões {tipo_botijao} na origem."}), 400
